...
export async function exportHighlights(): Promise<void> {
	try {
		...
		const fileName = `tolaria-clipper-highlights-${timestamp}.json`;
			...
			if (navigator.share) {
				try {
					await navigator.share({
						...
						title: 'Exported Tolaria Clipper Highlights',
						text: 'Here are your exported highlights from Tolaria Clipper.'
					});
				} catch (error) {
