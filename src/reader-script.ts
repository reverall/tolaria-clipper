...
(function() {
	...
	if (window.hasOwnProperty('tolariaReaderInitialized')) {
		return;  // Exit if already initialized
	}

	// Mark as initialized
	(window as any).tolariaReaderInitialized = true;
	...
	browser.runtime.onMessage.addListener((request: any, sender: browser.Runtime.MessageSender, sendResponse: (response?: any) => void) => {
		if (request.action === "toggleReaderMode") {
			...
			if (document.documentElement.classList.contains('tolaria-reader-active')) {
			...
			(async () => {
				try {
					...
					document.documentElement.classList.toggle('tolaria-reader-active', isActive);
