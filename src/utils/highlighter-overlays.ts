...
const IGNORED_BOUNDARY_SELECTOR =
	'.tolaria-highlighter-menu, .tolaria-reader-settings, .transcript-segment > strong, .tolaria-highlight-delete, .tolaria-selection-action';
...
const USER_HIGHLIGHT_NAME = 'tolaria-highlight';
...
function getTextSearchRoot(): Element {
	return document.querySelector('.tolaria-reader-content article')
...
function findOverlayAtPoint(x: number, y: number): HTMLElement | null {
	const overlays = document.querySelectorAll<HTMLElement>('.tolaria-highlight-overlay');
...
function ensureHighlightDeleteButton(): HTMLButtonElement {
	...
	btn.type = 'button';
	btn.className = 'tolaria-highlight-delete';
...
function handleHighlightClick(event: MouseEvent) {
	...
	if (target?.closest('.tolaria-highlight-delete, .tolaria-selection-action, a[href]')) return;
...
export function handleMouseUp(event: MouseEvent | TouchEvent) {
	...
	if (selection && !selection.isCollapsed) {
		...
		// NOTE: only works in reader mode (.tolaria-reader-content). On live
		// pages the content container isn't known, so this guard is a no-op.
		if (event instanceof MouseEvent) {
			const readerContent = document.querySelector('.tolaria-reader-content');
			if (readerContent) {
	...
	if (target.closest('.tolaria-highlight-delete, .tolaria-selection-action')) return;
...
export function planHighlightOverlayRects(target: Element | null, highlight: AnyHighlightData) {
	...
	overlay.className = 'tolaria-highlight-overlay';
	...
	if (atPoint && isDarkColor(getEffectiveBackgroundColor(atPoint as HTMLElement))) {
		overlay.classList.add('tolaria-highlight-overlay-dark');
	}
...
function updateHighlightOverlayPositions() {
	highlights.forEach((highlight) => {
		...
		if (!target) return;
		document.querySelectorAll(`.tolaria-highlight-overlay[data-highlight-id="${highlight.id}"]`)
...
const observer = new MutationObserver((mutations) => {
	...
	const shouldUpdate = mutations.some(m => {
		if (!(m.target instanceof Element) || m.target.id.startsWith('tolaria-highlight')) return false;
...
function handleHighlightHover(event: MouseEvent) {
	...
	requestAnimationFrame(() => {
		...
		const onButton = !!target?.closest('.tolaria-highlight-delete');
...
export function syncHoverListener(): void {
	const isActive = document.body.classList.contains('tolaria-highlighter-active');
...
export function removeExistingHighlights() {
	document.querySelectorAll('.tolaria-highlight-overlay').forEach(el => el.remove());
	clearTextHighlights();
	hideHighlightDeleteButton();
}