...
export function wireTranscript(
	...
	const pinToggle = createToggle(getMessage('readerPinPlayer'), pinDefault, (on) => {
		...
		window.dispatchEvent(new CustomEvent('tolaria-reader-show-nav'));
	...
	const updateActiveSegment = (currentTime: number) => {
		...
		if (newIndex !== activeIndex) {
			...
			if (chapter !== activeChapter) {
				if (activeChapter?.id) {
					doc.querySelector(`.tolaria-reader-outline-item[data-heading-id="${activeChapter.id}"]`)
						?.classList.remove('in-progress');
				}
				if (chapter?.id) {
					doc.querySelector(`.tolaria-reader-outline-item[data-heading-id="${chapter.id}"]`)
	...
	transcript.addEventListener('click', (e: MouseEvent) => {
		...
		if (doc.body.classList.contains('tolaria-highlighter-active')) return;
